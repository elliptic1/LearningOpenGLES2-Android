 ...
 */
class ShaderProgram(private val vertexShaderSource: String?, private val fragmentShaderSource: String?) {
    ...
    init {
        if (vertexShaderSource == null) throw IllegalArgumentException("vertex shader must not be null")
        if (fragmentShaderSource == null) throw IllegalArgumentException("fragment shader must not be null")
    ...
    fun getUniformLocation(name: String): Int {
        return if (uniforms[name] == null) -1 else uniforms[name] ?: -1
    }
    ...
    fun getAttributeLocation(name: String): Int {
        return if (attributes[name] == null) -1 else attributes[name] ?: -1
    }
